...
export const create_synopsis_comp = async (plot: any, project: any, project_title:any, project_type: any, setSynopsis: any, setProcessing: any, setProject:any) => {

  const pmt = `write synopsis for ${project?.genre?project.genre:""} ${project_type} titled ${project_title} with a plot based in ${location} where ${plot}`;
    ...
    await fetch('https://api.openai.com/v1/completions', requestOptions).then(res => res.json()).then(data => {
      create_res(pmt, data, "ad").then(res => {
        if (res && res.data && res.data[0].response) {
          ...
          if(res.data[0].response.choices[0].text && project?.id){
            const synopsis=res.data[0].response.choices[0].text
            update_project_synopsis(synopsis,project.id).then(res=>{
  ...
  await find_res(pmt).then(res => {
    if (res && res.existing_res && res.existing_res.length > 0 && res?.existing_res[0]?.response?.choices && res?.existing_res[0]?.response?.choices[0]?.text) {
      ...
      if(project.project_synopsis!==synopsis){
        update_project_synopsis(synopsis,project.id).then(res=>{
          if(res && res.data && res.data.length>0){
            setProject(res.data[0])
          }
