...
import { brahmiAnka } from "@/components/classes/brahmi"
...
export const Anka=()=>{
  ...
  const [set,setSet]=useState<any>(faker.helpers.arrayElements(brahmiAnka().filter(i=>i[2]!=='fi'||null),4))
  ...
  set.map((al:any,index:number)=>{
    ...
    onClick={()=>{
      if(al[2]===a[2]){
        ...
        setSet(faker.helpers.arrayElements(brahmiAnka().filter(i=>i[2]!=='fi'||''),4))
        setScore((s:any)=>s+5)
      } else {
        setWrong(true)
        setSuccess(false)
